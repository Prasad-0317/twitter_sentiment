...
# name_input = st.text_input("Enter Twitter handle, term, or hashtag:")
# mode_input = st.text_input("Enter mode (e.g., 'term', 'user', etc.):")
# number_input = st.number_input("Enter the number of tweets to fetch:")
# fetch_button = st.button("Fetch Tweets")

# if fetch_button:
#     if name_input:
#         # Fetch tweets based on user input
#         data = get_tweets(name_input, mode_input, number_input)  # Modify the mode and number as needed
#         st.write("### Fetched Tweets")
#         st.write(data)  # Display the fetched tweets in a DataFrame format
#     else:
#         st.warning("Please enter a valid input")

# data = get_tweets(name_input, mode_input, number_input)
...
# Convert entire column to datetime objects
df['date'] = pd.to_datetime(df['date'], format='%b %d, %Y · %I:%M %p UTC')

# Convert datetime column to the desired format
df['date'] = df['date'].dt.strftime('%d-%m-%Y')

# Display the DataFrame with the new date format
# print(df)

# df.to_excel('formatted_dates.xlsx', index=False)
...
# Define a function to label the text
def label_text(text):
  # You can use a sentiment analysis library to generate a sentiment score for the text
...
plt.pie([positive_count, negative_count,neutral_count], labels=['Positive', 'Negative','Neutral'], autopct="%1.1f%%",radius=1)
